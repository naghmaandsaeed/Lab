...
char *INSTUCTIONS[] = {".data", ".string", ".extern", ".entry", "define"};
...
int legal_index(char *str, label_address *label_table, int table_lines){
	char *token , *num;
	...
	num = strtok(NULL,"]");
	if(is_const(num,label_table,table_lines) != NULL)
	    strcpy(num,is_const(num,label_table,table_lines)->address);
...
label_address is_const(char *str, label_address *label_table , int table_lines)
{
	int i;
	/* Iterate through each label in the label_table */
	for (i = 0; i < table_lines - 1; i++) {
		if(strcmp((label_table + i)->type,"mdefine") == 0 && strcmp((label_table + i)->label_name , str))
			return (label_table + i);
	}
	return NULL;
}

int is_arg_num_or_const(char *str){
	if (str == NULL)
	        return 0;
...
int is_reg_or_label_or_num(char *str) {
    ...
    return (is_reg_or_label(str) || is_arg_num_or_const(str));
}
...
int legal_arg(char *str, command_parts *command, int *error_code, label_address *label_table, int table_lines) {
    char *str1, *str2, *ptr;
    ...
    if (OPCODES[command->opcode].arg_num == 2) {
        if (strstr(str, ",") == NULL) {
            ...
            *error_code = ERROR_CODE_39; /* Extra commas between arguments */
            ...
            if(*str1 == '#'){
            	str1++;
            	if(is_const(str1,label_table,table_lines) != NULL)
            	    strcpy(str1,is_const(str1,label_table,table_lines)->address);
            }
            ...
            if(*str2 == '#'){
            	str2++;
            	if(is_const(str2,label_table,table_lines) != NULL)
            	    strcpy(str2,is_const(str1,label_table,table_lines)->address);
            }
        }
    }
    /* Handle cases where the command has one argument */
        ...
        if(*str == '#'){
        	str++;
        	if(is_const(str,label_table,table_lines) != NULL)
        	    strcpy(str,is_const(str1,label_table,table_lines)->address);
        }
    }
    ...
    switch (command->opcode) {
        ...
        case 1: {
            if ((is_reg_or_label_or_num(str1) || legal_index(str1, label_table, table_lines)) && (is_reg_or_label_or_num(str2) || legal_index(str1, label_table, table_lines))) {
                command->source = str1;
                command->dest = str2;
            } else {
                if (str2 == NULL)
        ...
        case 2:
        case 3: {
            if ((is_reg_or_label_or_num(str1) || legal_index(str1,label_table, table_lines)) && (is_reg_or_label(str2) || legal_index(str2,label_table, table_lines))) {
                command->source = str1;
                command->dest = str2;
            } else {
        ...
        /* source addressing code is 1,2 and dest addressing code is 1,2,3 */
        case 6: {
            if ((legal_label(str1) || legal_index(str1,label_table, table_lines)) && (is_reg_or_label(str2) || legal_index(str2,label_table, table_lines))) {
                command->source = str1;
                command->dest = str2;
            } else {
                *error_code = ERROR_CODE_33; /* Illegal argument */
        ...
        case 8:
        case 11:{
            if (is_reg_or_label(str) || legal_index(str,label_table, table_lines)) {
                command->source = NULL;
                command->dest = str;
            } else {
        ...
        /* dest addressing code is 1,3 and no source */
        case 9:
        case 10:
        case 13: {
        ...
        case 12: {
            if (is_reg_or_label_or_num(str) || legal_index(str,label_table, table_lines)) {
                command->source = NULL;
                command->dest = str;
            } else {
...
int capture_nums(char *str, char *token_copy, inst_parts *inst, int *error_code, label_address *label_table, int table_lines) {
    char *token;
    int len, number;
    len = 0;
    ...
    {
    	if(is_const(token,label_table,table_lines) != NULL)
    		strcpy(token,is_const(token,label_table,table_lines)->address);
    }
...
inst_parts *read_const(char *str, int *error_code) {
	inst_parts *inst;
	char *token;
	char *constant_name, *constant_value;
    ...
    strtok(str," \n");
	/* Skip the initial ".define" directive */
	token = strtok(NULL, " \n");
	if (token == NULL) {
		*error_code = ERROR_CODE_60;
		return 0;
	}
	...
	token = strtok(NULL, " \n");
	if (token == NULL) {
		*error_code = ERROR_CODE_61;
		return 0;
	}
	constant_value = token;

	/*Check if the constant name is legal*/
	if(!legal_label(constant_name))
		*error_code = ERROR_CODE_62;
	else inst->label = constant_name;

	/*Check if the constant value is legal*/
	if(!is_num(constant_value))
		*error_code = ERROR_CODE_63;
	else inst->const_value = constant_value;
...
inst_parts *read_instruction(char *str, int *error_code, label_address *label_table, int table_lines) {
    inst_parts *inst;
    char *token;
    ...
    if (legal_label_decl(token, error_code)) {
        ...
        token = strtok(NULL, " \n");
            ...
            strcmp(token, ".entry") == 0 || strcmp(token, ".extern") == 0 || strcmp(token, ".define") == 0) {
        inst->label = NULL;
    }

    /* Check directive cases */
    if (strcmp(token, ".data") == 0) {
        capture_nums(str,token_copy,inst,error_code,label_table,table_lines);
    } else if (strcmp(token, ".string") == 0) {
        capture_string(str, inst, error_code);
    }else if (strcmp(token, ".define") == 0) {
        read_const(str, error_code);
...
command_parts *read_command(char *str, int *error_code, label_address *label_table, int table_lines) {
    char *token;
    int flag_visited = 0;
    ...
    if (legal_label_decl(token, error_code)) {
        ...
        if (OPCODES[command->opcode].arg_num == 0) {
            ...
            if (legal_arg(strtok(NULL, "\n"), command, error_code, label_table, table_lines) == 0) {
                return command;
            }
        }
    }
