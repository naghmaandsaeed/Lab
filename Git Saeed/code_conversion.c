...
unsigned short command_to_short(command_parts *command) {
	...
	else if(legal_index(command->source)) {
		n_src = (short) (INDEX_ADDRESSING << SOURCE_BITS_SHIFT);  /* Fill in index addressing bits. */
	}
    ...
    if (what_reg(command->dest) >= 0) {
        ...
        n_src = (short) (DIRECT_ADDRESSING << DEST_BITS_SHIFT);  /* Fill in direct addressing bits. */
    }
	else if(legal_index(command->dest)) {
		n_src = (short) (INDEX_ADDRESSING << DEST_BITS_SHIFT);  /* Fill in index addressing bits. */
	}
    /* Set the opcode bits. */
...
int add_extra_machine_code_line(code_conv **code, command_parts *command, int *IC, int is_src, location am_file) {
    ...
    /* Check if the argument is a numerical value */
    else if (is_arg_num_or_const(arg)) {
        (*IC)++;
        /* representing number in 2-11 bits, therefore pushing the number ARE_BITS bits to the left */
        if (add_machine_code_line(code, atoi(arg + 1) << ARE_BITS, NULL, IC, am_file) == 0) {
            return 0;
        }
    }
    else if(legal_index(arg)){
    	(*IC)++;
    	arg = strtok(arg,"[");
    	index = strtok(NULL,"]");
        ...
        (*IC)++;
        if (add_machine_code_line(code, atoi(index) << ARE_BITS, NULL, IC, am_file) == 0) {
            return 0;
        }
    }
