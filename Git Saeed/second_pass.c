...
int conv_code_base4_modified(code_conv *code, int count, char *file_name, int IC, int DC) {
    int i;
    FILE *fp;
    ...
    for (i = 0; i <= count; i++) {
        base4_char = short_to_base4_modified((code + i)->short_num);
        fprintf(fp, "%d %s\n", IC_INIT_VALUE + i, base4_char);
        free(base4_char);
    }
...
int exe_second_pass(char *file_name, label_address *label_table, int IC, int DC, int label_table_line, \
    ...
    change_label_table_data_count(label_table, label_table_line, IC + IC_INIT_VALUE);
    ...
    if (error_found == 0) {
        conv_code_base4_modified(code, IC + DC, file_name, IC, DC);
