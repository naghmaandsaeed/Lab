...
int insert_other_labels(other_table **table, int count, inst_parts *inst, char *type, location am_file, int *error_code) {
    other_table *ptr;
    int label_length;
	...
	(*table + count - 1)->type = handle_malloc((strlen(type) + 1) * sizeof(char));
    if ((*table + count - 1)->label_name == NULL || (*table + count - 1)->type == NULL) {
        *error_code = ERROR_CODE_1;
        return 0;
    }
    /*Copy the data into the table*/
    strcpy((*table + count - 1)->label_name, inst->arg_label);
	strcpy((*table + count - 1)->type, inst->type);
...
insert_label_table(label_address **label_table, int lines, char *label, int counter, location am_file, int is_data_line,
    ...
    *label_table = realloc(*label_table, lines * sizeof(label_address));
    ...
    /* Allocate memory for the label_name and type and copy the label and type from the input */
    (*label_table + lines - 1)->label_name = malloc((strlen(label) + 1) * sizeof(char));
	(*label_table + lines - 1)->type = malloc((strlen(type) + 1) * sizeof(char));
    if ((*label_table + lines - 1)->label_name == NULL || (*label_table + lines - 1)->type == NULL) {
        *error_code = ERROR_CODE_1;
        return 0;
    }
    strcpy((*label_table + lines - 1)->label_name, label);
	strcpy((*label_table + lines - 1)->type, type);
...
void change_label_table_data_count(label_address *label_table, int table_lines, int IC) {
    ...
    for (i = 0; i < table_lines; i++) {
        ...
        if (strcmp((label_table + i)->type ,".data") == 0) {
